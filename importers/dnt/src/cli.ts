import { ingestAll, ingestAssociation } from './ingest.ts';
import { associationBySlug } from './associations.ts';

/**
 * Entry point for the scheduled job.
 *   pnpm --filter @hendingar/importer-dnt ingest [--dry-run] [--only <slug>]
 */
...
try {
	let results;
	if (only) {
		const association = associationBySlug(only);
		if (!association) {
			console.error(`unknown turlag '${only}'`);
			process.exit(1);
		}
		results = [await ingestAssociation(url, association, { dryRun, trigger, revision })];
	} else {
