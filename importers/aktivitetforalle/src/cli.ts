import { ingestAll, ingestSite } from './ingest.ts';
import { siteBySlug } from './sites.ts';

/**
 * Entry point for the scheduled job.
 *   pnpm --filter @hendingar/importer-aktivitetforalle ingest [--dry-run] [--only <slug>]
 */
...
try {
	let results;
	if (only) {
		const site = siteBySlug(only);
		if (!site) {
			console.error(`unknown site '${only}'`);
			process.exit(1);
		}
		results = [await ingestSite(url, site, { dryRun, trigger, revision })];
	} else {
