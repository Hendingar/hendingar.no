import { ingestAll, ingestCampus } from './ingest.ts';
import { campusBySlug } from './campuses.ts';

/**
 * Entry point for the scheduled job.
 *   pnpm --filter @hendingar/importer-hvl ingest [--dry-run] [--only <slug>]
 */
...
try {
	let results;
	if (only) {
		const campus = campusBySlug(only);
		if (!campus) {
			console.error(`unknown campus '${only}'`);
			process.exit(1);
		}
		results = [await ingestCampus(url, campus, { dryRun, trigger, revision })];
	} else {
