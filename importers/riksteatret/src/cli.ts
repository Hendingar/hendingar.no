 ...
 *   pnpm --filter @hendingar/importer-riksteatret ingest [--dry-run] [--only <slug>]
 */
...
try {
	...
	if (only) {
		...
		if (!instance) {
			console.error(`unknown venue '${only}'`);
			process.exit(1);
		}
	...
	// A partial run is not a failure — some cards changed shape and the rest imported fine — but it
	// must be visible in the job output. Any outright failure fails the job.
