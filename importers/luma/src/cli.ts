import { ingestAll, ingestCalendar } from './ingest.ts';
import { calendarBySlug } from './calendars.ts';

/**
 * Entry point for the scheduled job.
 *   pnpm --filter @hendingar/importer-luma ingest [--dry-run] [--only <slug>]
 */
...
try {
	let results;
	if (only) {
		const calendar = calendarBySlug(only);
		if (!calendar) {
			console.error(`unknown calendar '${only}'`);
			process.exit(1);
		}
		results = [await ingestCalendar(url, calendar, { dryRun, trigger, revision })];
	} else {
