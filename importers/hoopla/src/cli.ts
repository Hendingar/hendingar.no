import { ingestAll, ingestShop } from './ingest.ts';
import { shopBySlug } from './shops.ts';

/**
 * Entry point for the scheduled job.
 *   pnpm --filter @hendingar/importer-hoopla ingest [--dry-run] [--only <slug>]
 */
...
try {
	let results;
	if (only) {
		const shop = shopBySlug(only);
		if (!shop) {
			console.error(`unknown shop '${only}'`);
			process.exit(1);
		}
		results = [await ingestShop(url, shop, { dryRun, trigger, revision })];
	} else {
