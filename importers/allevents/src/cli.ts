import { ingestAll, ingestOrganiser } from './ingest.ts';
import { organiserBySlug } from './organisers.ts';

/**
 * Entry point for the scheduled job.
 *   pnpm --filter @hendingar/importer-allevents ingest [--dry-run] [--only <slug>]
 */
...
try {
	let results;
	if (only) {
		const organiser = organiserBySlug(only);
		if (!organiser) {
			console.error(`unknown organiser '${only}'`);
			process.exit(1);
		}
		results = [await ingestOrganiser(url, organiser, { dryRun, trigger, revision })];
	} else {
