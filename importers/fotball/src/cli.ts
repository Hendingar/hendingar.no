import { ingestAll, ingestTeam } from './ingest.ts';
import { teamBySlug } from './teams.ts';

/**
 * Entry point for the scheduled job.
 *   pnpm --filter @hendingar/importer-fotball ingest [--dry-run] [--only <slug>]
 */
...
try {
	let results;
	if (only) {
		const team = teamBySlug(only);
		if (!team) {
			console.error(`unknown team '${only}'`);
			process.exit(1);
		}
		results = [await ingestTeam(url, team, { dryRun, trigger, revision })];
	} else {
