...
{#if photoEnabled}
	...
	<!-- Both paths end in the same form. The divider says so, rather than letting the photo panel
	     look like a prerequisite. -->
	<p class="or" aria-hidden="true"><span>eller skriv det inn sjølv</span></p>
{/if}
...
<form {...submitEvent} class="form frame">
	<div class="form__intro">
		<p class="label">Skjema</p>
		<h2 class="display display--md">Skriv det inn</h2>
		{#if method === 'photo'}
			<p class="form__read">
				Fylt ut frå plakaten. Les gjennom og rett det som er feil — ingenting blir sendt før du
				trykkjer send.
			</p>
		{/if}
	...
	<fieldset class="group">
		...
		<p class="group__hint">Dato og klokkeslett slik dei står på plakaten — lokal tid.</p>
		<div class="grid">
			<p class="field">
